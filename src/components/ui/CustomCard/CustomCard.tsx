...
import { EDataType, EIsEncrypted } from 'store/enums';

export interface ICustomCard {
    title?: string;
    img?: string;
    text?: string;
    badgeState?: EIsEncrypted;
    badgeText?: string;
    isVertical?: boolean;
    id?: number;
    buttonsArr?: string[];
...
const CustomCard: FC<ICustomCard> = memo(({title = "", img = "", text = "", badgeState = false, isVertical = true, buttonsArr = [], badgeText = '', textType = EDataType.TEXT, id = -1 , onClick = () => null, withLoader = false }) => {
    const rowSize = useMemo(() => isVertical ? '12' : 1, [isVertical]);
    const imgColSize = useMemo(() => isVertical ? '12' : 6, [isVertical]);
    const badgeBg =  useMemo(() => badgeState === EIsEncrypted.false ? EBootstrapColor.DANGER : EBootstrapColor.SUCCESS, [badgeState]);
